...
trControl <- trainControl(method='cv', number=5) # 5회 교차검증

fit <- train(Species ~ ., method='knn', tuneGrid=expand.grid(k=1:10), # tuneGrid: k값을 1~10까지 테스트
	trControl=trControl, metric='Accuracy', data=iris)
fit
...
for (i in 1:5){
	...
	# knn 모형
	pred <- knn(ds.tr, ds.te, cl.tr, k=8)
